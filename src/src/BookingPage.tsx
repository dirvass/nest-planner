...
// Example blocked dates (booked). Add your real inventory here.
...
const CLEANING_FEE = 150;   // EUR – once per stay
const SERVICE_FEE_PCT = 0.05; // 5% service
const REFUNDABLE_DEPOSIT = 500; // EUR – taken on arrival, not included in “Payable now”
      ...
      <main className="container">
        <div className="card" style={{ display: "grid", gridTemplateColumns: "1.1fr 0.9fr", gap: 24 }}>
          ...
          <section>
            ...
            {/* DayPicker with booked ranges disabled */}
            <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 16, padding: 12 }}>
              ...
              <div className="helper">
                Dates shown in grey are unavailable. Minimum stay 3 nights in peak periods.
              </div>
            </div>
          ...
          <aside className="card" style={{ alignSelf: "start" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", marginTop: 6 }}>
              <tbody>
                <tr>
                  <td>Accommodation</td>
                  <td style={{ textAlign: "right" }}>€ {price.base.toFixed(2)}</td>
                </tr>
                <tr>
                  <td>Cleaning fee</td>
                  <td style={{ textAlign: "right" }}>€ {price.cleaning.toFixed(2)}</td>
                </tr>
                <tr>
                  <td>Service {Math.round(SERVICE_FEE_PCT * 100)}%</td>
                  <td style={{ textAlign: "right" }}>€ {price.service.toFixed(2)}</td>
                </tr>
                <tr>
                  <td style={{ paddingTop: 6, fontWeight: 700 }}>Total</td>
                  <td style={{ textAlign: "right", paddingTop: 6, fontWeight: 700 }}>€ {price.total.toFixed(2)}</td>
                </tr>
                <tr>
                  <td colSpan={2} style={{ color: "#64748b", fontSize: 12, paddingTop: 6 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
              {/* Replace phone with your concierge WhatsApp number */}
              <a
                className="btn primary"
                href={`https://wa.me/00000000000?text=${waText}`}
                target="_blank"
                rel="noreferrer"
              ...
              <a
                className="btn ghost"
                href={`mailto:reservations@nest-ulasli.com?subject=Booking Enquiry – ${villaInfo.name}&body=${waText}`}
                style={{ flex: 1, textAlign: "center" }}
              >
                Request to Book
              </a>
